...
class BossBar(HealthBar):
    def __init__(self, x, y, w, h, max_hp):
        ...
        # Load images
        self.contour_image = pygame.image.load(MOMO_HEALTH_CONTOUR).convert_alpha()
        self.healthbar_image = pygame.image.load(MOMO_HEALTH_FILL).convert_alpha()

        # Scale images
        self.contour_image = pygame.transform.scale(self.contour_image,
            (int(self.contour_image.get_width() * MOMO_HEALTHBAR_SCALE_FACTOR), int(self.contour_image.get_height() * MOMO_HEALTHBAR_SCALE_FACTOR)))
        self.healthbar_image = pygame.transform.scale(self.healthbar_image,
            (int(self.healthbar_image.get_width() * MOMO_HEALTHBAR_SCALE_FACTOR), int(self.healthbar_image.get_height() * MOMO_HEALTHBAR_SCALE_FACTOR)))
