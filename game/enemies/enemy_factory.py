    ...
    def __call__(self, type : str, spritesheet, speed, scale, health, attack_damage, attack_range, wander_time, x, y) -> Enemy:
        if 'slime' in type:
            return SlimeEnemy(sprites_group = self.sprites_group, spritesheet=spritesheet, wander_time=wander_time,
                              colisionHandler=self.colisionHandler, frame_width=32, frame_height=32, attack_damage=attack_damage, attack_range=attack_range, num_frames=4, x=x, y=y, speed=speed, attack_type='mele', scale=scale, health=health, player = self.player, colision_group= self.collision_group)
        if 'skeleton' in type:
            return SkeletonEnemy(sprites_group = self.sprites_group, spritesheet=spritesheet, wander_time=wander_time,
                              colisionHandler=self.colisionHandler, frame_width=16, frame_height=16, attack_damage=attack_damage, attack_range=attack_range, num_frames=4, x=x, y=y, speed=speed, attack_type='mele', scale=scale, health=health, player = self.player, colision_group= self.collision_group)
