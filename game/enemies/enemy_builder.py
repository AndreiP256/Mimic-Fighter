...
class EnemyBuilder:
    def __init__(self, player:Player, colisionHandler):
        self.player = player
    ...
    def create_enemy(self, enemy_type, x, y):
        if enemy_type in self.enemy_types:
            ...
            return SlimeEnemy(spritesheet=spritesheet, wander_time=wander_time, colisionHandler=self.colisionHandler, frame_width=32, frame_height=32, attack_damage=attack_damage, attack_range=attack_range, num_frames=4, x=x, y=y, speed=speed, attack_type='mele', scale=scale, health=health, player = self.player)
        else:
